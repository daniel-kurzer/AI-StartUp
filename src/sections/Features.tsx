...
const FeatureTab = (props: (typeof tabs)[number] & ComponentPropsWithoutRef<'div'> & { selected: boolean}) => {
  ...
  return (
    <div ref={tabRef} onClick={props.onClick} onMouseEnter={handleTabOver} className="border border-white/15 flex p-2.5 rounded-xl lg:flex-1 gap-2.5 items-center relative cursor-pointer hover:scale-105 transition hover:text-white/70">
      {props.selected && (
        <motion.div style={{maskImage,}} className="absolute -m-px inset-0 border border-[#A369FF] rounded-xl"></motion.div>
      )}
      <div className="h-12 w-12 border border-white/15 rounded-lg inline-flex items-center justify-center"><DotLottiePlayer ref={dotLottieRef} src={props.icon} className="h-5 w-5" autoplay loop />
      <div className="h-7 w-7 blur animate-pulse absolute -z-10 bg-purple-500 rounded-full flex justify-center"></div></div>
      <div className="font-medium">{props.title}</div>
      {props.isNew && (<div className="text-xs text-white/50 border border-white/20 font-semibold animate-pulse rounded-full px-2 py-0.5 bg-[#8c44ff]">new</div>)}
    </div>
  )
}
